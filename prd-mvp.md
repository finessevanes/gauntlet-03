...
- **Frontend**: React
- **Media Processing**: FFmpeg (bundled with app)
...
- **Error Handling**: FFmpeg missing/corrupted → show error dialog with reinstall instructions
...
**REQ-2.2**: Support file picker (File > Import or button) filtered to `.mp4` and `.mov` only  
**REQ-2.3**: Reject unsupported formats before selection (file picker shows only MP4/MOV)  
**REQ-2.4**: Display imported clips in Library panel with thumbnails and duration  
**REQ-2.5**: Allow duplicate imports (same file can be added multiple times)  
...
**REQ-7.1**: Export button triggers save file dialog  
...
**Cancellation**:
**REQ-7.6**: Cancel button stops export immediately  
**REQ-7.7**: Cleanup partial output file on cancel  
...
- **Error Handling**: State file corrupted → launch with blank slate (log error)
...
2. Launch Klippy, note baseline memory (e.g., ~150MB)
...
**Pass Criteria**:
- Memory stabilizes within ±100MB after initial clip loading
- No continuous growth (e.g., NOT 600MB → 1.2GB → 2GB)
- App remains responsive (UI actions <100ms)

**Fail Signs**:
- Memory grows >100MB every 3 minutes
...
3. Drag clips to timeline in order: A → B → C
...
**Current Project Structure** (Tauri + React scaffold):

```
gauntlet-03 (Klippy App)/
...
│   ├── tauri.conf.json
│   ├── capabilities/
│   │   └── default.json (Tauri security config)
│   ├── icons/
...
1. **Import**: File picker → Validate format → Extract metadata (duration, resolution) → Add to Library
2. **Timeline Edit**: User action → Update React state → Re-render timeline
3. **Preview**: Playhead position → Calculate clip + offset → Seek `<video>` element
4. **Export**: Timeline state → Generate FFmpeg command → Execute → Monitor progress → Save file

### FFmpeg Integration

**Bundling Strategy**: Include FFmpeg binary in Tauri resources folder. Extract to temp directory on first run.
...
- Monitor stderr for progress percentage
...
**Phase 1: Foundation**
- Tauri app setup + basic UI scaffold
- File import + Library view
- FFmpeg integration (validate can run commands)
...
- Export pipeline with progress indicator
