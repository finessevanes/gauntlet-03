...
Specify concrete methods the frontend calls.

**Example (Tauri command):**
```typescript
await invoke('import_video', { filePath: '/path/to/video.mp4' });
...
List React components and Tauri commands with one-line purpose.

- `src/components/Library.tsx` — [purpose]
- `src/components/Timeline.tsx` — [purpose]
- `src-tauri/src/commands/video.rs` — [purpose]

---

## 8. Integration Points

- File system access (Tauri fs module)
- FFmpeg (if applicable)
- Window/canvas rendering
- State management (Context, Zustand, Redux)
- Desktop lifecycle (window events, app close)
...
- Keep Rust backend deterministic, React UI thin wrapper
